...
const BodyMapInner: React.FC<BodyMapProps> = ({ onPartClick, selectedPart, muscleVolumes, maxVolume = 1, onPartHover }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const volumeColorsRef = useRef<Map<string, string>>(new Map());
  const currentHoverRef = useRef<string | null>(null);

  // Helper to apply base volume color to a muscle
  const applyMuscleColor = useCallback((el: Element, color: string) => {
    const paths = el.querySelectorAll('path');
    paths.forEach(path => {
      const svgPath = path as SVGElement;
      svgPath.style.fill = color;
      svgPath.style.transition = 'fill 0.15s ease, filter 0.15s ease';
      svgPath.style.filter = '';
    });
    (el as SVGElement).style.color = color;
  }, []);

  // Helper to highlight a muscle (subtle brightness increase)
  const highlightMuscle = useCallback((el: Element) => {
    const paths = el.querySelectorAll('path');
    paths.forEach(path => {
      (path as SVGElement).style.filter = 'brightness(1.3) saturate(1.2)';
    });
  }, []);

  // Helper to apply selected style (blue)
  const applySelectedStyle = useCallback((el: Element) => {
    const paths = el.querySelectorAll('path');
    paths.forEach(path => {
      const svgPath = path as SVGElement;
      svgPath.style.fill = '#3b82f6'; // blue-500
      svgPath.style.filter = '';
    });
    (el as SVGElement).style.color = '#3b82f6';
  }, []);

  // Apply all base colors (called once when muscleVolumes change)
  const applyAllColors = useCallback(() => {
    if (!containerRef.current || !muscleVolumes) return;
    
    volumeColorsRef.current.clear();
    const bodymapElements = containerRef.current.querySelectorAll('.bodymap');
    
    bodymapElements.forEach(el => {
      const muscleId = el.id;
      const volume = muscleVolumes.get(muscleId) || 0;
      const color = getVolumeColor(volume, maxVolume);
      volumeColorsRef.current.set(muscleId, color);
      
      // Apply color unless this is the selected part
      if (muscleId !== selectedPart) {
        applyMuscleColor(el, color);
      } else {
        applySelectedStyle(el);
      }
    });
  }, [muscleVolumes, maxVolume, selectedPart, applyMuscleColor, applySelectedStyle]);

  // Apply colors when muscleVolumes change
  useEffect(() => {
    applyAllColors();
  }, [applyAllColors]);

  // Handle selected part styling
  useEffect(() => {
    if (!containerRef.current) return;
    
    // Reset all to volume colors first
    const bodymapElements = containerRef.current.querySelectorAll('.bodymap');
    bodymapElements.forEach(el => {
      const muscleId = el.id;
      const color = volumeColorsRef.current.get(muscleId) || getVolumeColor(0, 1);
      applyMuscleColor(el, color);
    });
    
    // Apply selected style
    if (selectedPart) {
      const parts = containerRef.current.querySelectorAll(`[id="${selectedPart}"].bodymap`);
      parts.forEach(part => {
        applySelectedStyle(part);
      });
    }
  }, [selectedPart, applyMuscleColor, applySelectedStyle]);

  const handleContainerClick = useCallback((e: React.MouseEvent) => {
    const target = (e.target as Element).closest('.bodymap');
    if (target && target.id) {
      onPartClick(target.id);
    }
  }, [onPartClick]);

  // Direct DOM hover handling - no state updates, no re-renders
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    if (!containerRef.current) return;
    
    const target = (e.target as Element).closest('.bodymap');
    const newHoverId = target?.id || null;
    
    // Skip if same element
    if (newHoverId === currentHoverRef.current) return;
    
    // Remove highlight from previous hover (if not selected)
    if (currentHoverRef.current && currentHoverRef.current !== selectedPart) {
      const prevElements = containerRef.current.querySelectorAll(`[id="${currentHoverRef.current}"].bodymap`);
      prevElements.forEach(el => {
        const color = volumeColorsRef.current.get(currentHoverRef.current!) || getVolumeColor(0, 1);
        applyMuscleColor(el, color);
      });
    }
    
    // Add highlight to new hover (if not selected)
    if (newHoverId && newHoverId !== selectedPart) {
      const newElements = containerRef.current.querySelectorAll(`[id="${newHoverId}"].bodymap`);
      newElements.forEach(el => {
        highlightMuscle(el);
      });
    }
    
    currentHoverRef.current = newHoverId;
    
    // Notify parent (for info panel) without causing re-render here
    if (onPartHover) {
      onPartHover(newHoverId);
    }
  }, [selectedPart, onPartHover, applyMuscleColor, highlightMuscle]);

  const handleMouseLeave = useCallback(() => {
    if (!containerRef.current) return;
    
    // Remove highlight from current hover (if not selected)
    if (currentHoverRef.current && currentHoverRef.current !== selectedPart) {
      const prevElements = containerRef.current.querySelectorAll(`[id="${currentHoverRef.current}"].bodymap`);
      prevElements.forEach(el => {
        const color = volumeColorsRef.current.get(currentHoverRef.current!) || getVolumeColor(0, 1);
  ...
  }, [selectedPart, onPartHover, applyMuscleColor]);

  return (
    <div
      ref={containerRef}
      className="body-map-container w-full h-full flex justify-center items-center"
...
export const BodyMap = memo(BodyMapInner, (prevProps, nextProps) => {
  // Re-render if these props change
  if (prevProps.selectedPart !== nextProps.selectedPart) return false;
  if (prevProps.maxVolume !== nextProps.maxVolume) return false;
  
  // Deep compare muscleVolumes map
  if (prevProps.muscleVolumes !== nextProps.muscleVolumes) {
    if (!prevProps.muscleVolumes || !nextProps.muscleVolumes) return false;
    if (prevProps.muscleVolumes.size !== nextProps.muscleVolumes.size) return false;
    for (const [key, value] of prevProps.muscleVolumes) {
      if (nextProps.muscleVolumes.get(key) !== value) return false;
    }
  }
  
  // Don't re-render for callback or hover changes (hover handled via direct DOM)
  return true;
});
