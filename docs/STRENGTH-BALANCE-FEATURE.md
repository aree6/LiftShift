...
- Warm-up sets and **future-dated sets** (a mis-dated import, a typo in a
  date field) are excluded when choosing the anchor, so one rogue set can't
...
ratio of that week's best e1RMs is recorded as `history = [{weekStart, ratio}, …]`,
...
4. Gender-specific lateral-raise bands (StrengthLevel has ~0.15×BW female
   vs. ~0.20×BW male tables), and the OHP:bench tables already agree across
   sexes at about 64%, so only some pairs would need them.
